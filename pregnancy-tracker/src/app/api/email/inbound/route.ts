...
export async function POST(request: Request) {
  try {
    ...
    const user = await users.findOne({ email: fromEmail });
    ...
    if (text.includes('labor started') && user.status === 'yes') {
      await users.updateOne({ _id: user._id }, { $set: { status: 'in progress', laborStart: eventTime } });
      ...
      await users.updateOne({ _id: user._id }, { $set: { status: 'no', born: eventTime } });
